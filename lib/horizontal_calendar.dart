...
class HorizontalCalender2 extends StatefulWidget {
  const HorizontalCalender2({super.key, required this.onSelected, this.child, this.header});
  ...
  @override
  State<HorizontalCalender2> createState() => _HorizontalCalender2State();
}

class _HorizontalCalender2State extends State<HorizontalCalender2> {
      ...
      children: [
        ...
        SizedBox(
          ...
          child: PageView.builder(
              ...
              onPageChanged: (page) {
                scrollMonthYear = DateFormat('MMMM, yyyy').format(dateList[page].last);
                setState(() {});
              },
                        ...
                        .map(
                          (date) => GestureDetector(
                            ...
                            child: widget.child != null
                                ? widget.child!(date, DateTime(date.year, date.month, date.day).isAtSameMomentAs(currentSelectedDate))
                                : CalenderItemWidget(
                                    date: date,
                                    isSelected: DateTime(date.year, date.month, date.day).isAtSameMomentAs(currentSelectedDate),
                                  ),
                          ),
                        )
                        .toList(),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: isSelected ? const Color(0xFF144522) : Colors.transparent,
      ),
      child: Column(
        ...
        children: [
          Text(
            "${date.day}",
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isSelected ? AppColor.basicWhite : AppColor.basicBlack),
          ),
          Text(
            DateFormat('E').format(date),
            style: TextStyle(fontSize: 12, color: isSelected ? AppColor.basicWhite : AppColor.basicBlack),
          ),
        ],
      ),
    );
